...
Except for the *Command-query separation*, the other ones require an object-oriented language to be used. We will explore in the following deliverables whether those kinds of measures can also be applied to trait-based languages like Rust.
...
Thus a function that answers a question should return a value and not alter the data state, so running a command. Conversely, if a function runs a command, it should alter the data state without returning a value. For maximum clarity, a function *should* never return a value and alter the data state.

This kind of separation can clarify intent and prevent errors. As functions and code bases become large, separation becomes much more important, as hunting for the function definition to find out what it does is not an efficient use of time.
...
Nevertheless, the tight coupling can also be present when a determined class needs to be modified because another one changes, usually when a class depends on methods or properties taken from other classes. For example, not to break the code when new parameters are added to a class, a *loose coupling* way could be passing the constructor parameters as an object with the receiving object having fallback default values.

A good code should be built as a series of the independent block, easily connectable one to each other, and not as a series of intertwined pieces.
...
COGNITIVE
: Cognitive Complexity. It is a measure of how difficult a unit of code is to understand intuitively. 
...
Finally, we present a composite metric to provide a single index of maintainability for software [@oman1992metrics], called *Maintainability Index*. 
...
Three distinct variants of this metric are considered. The original formula, created by Oman et al.., then the one defined by the Software Engineering Institute (*SEI*) and promoted in the C4 Software Technology Reference Guide [@bray1997c4], and finally the variant implemented for the Visual Studio IDE [@mi-vs]. 
